...
namespace dae {
	...
	{
		// range to norm the z value -> with near and far plane
		float A = zf / (zf - zn);
		float B = -(zf * zn) / (zf - zn);

		Matrix matrix{	// this will be different if using the right ahnded system!
			{1 / (aspect * fov),	0,				0,		0},
			{0,						1 / fov,		0,		0},
			{0,						0,				A,		1},			// 1 => to store original z component
			{0,						0,				B,		0}
		};
